...
export function WhatsAppFloat() {
  const phone = (process.env.NEXT_PUBLIC_STORE_WHATSAPP || "").replace(/\D/g, "");

  if (!phone) return null;

  const text = encodeURIComponent(
    "Olá! Vim pelo site da Cuias Lazaretti e gostaria de atendimento."
  );

  const href = `https://wa.me/55${phone}?text=${text}`;
  ...
  return (
    <a
      ...
      className="fixed bottom-5 right-5 z-50 inline-flex h-14 w-14 items-center justify-center rounded-full bg-[#25D366] text-white shadow-xl transition hover:scale-105 hover:shadow-2xl"
    >
      <svg
